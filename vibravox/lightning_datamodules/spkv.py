...
class SPKVLightningDataModule(LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage == "fit" or stage is None:
            ...
            if self.sensorA == self.sensorB:
                ...
                train_dataset = interleave_datasets(datasets=[train_dataset_a, train_dataset_b],
        ...
        if stage == "test":
            ...
            pickle_folder = f"configs/lightning_datamodule/spkv_pairs/{self.dataset_name.split('/')[1]}/{self.subset}/{self.pairs}.pkl"
            with open( Path(__file__).parent.parent.parent /pickle_folder, 'rb') as file:
    ...
    def data_collator(self, batch: List[Dict[str, Union[torch.Tensor, str, int, str, str]]]) -> Dict[str, Union[torch.Tensor, List[str], List[int], List[str], List[str]]]:
        """
            ...
            Parameters:
                batch (List[Dict[str, Union[torch.Tensor, str, int, str, str]]]): List of dictionaries with keys 'audio', 'speaker_id', 'sentence_id', 'gender', and 'sensor'
