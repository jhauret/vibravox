...
```
python run.py lightning_datamodule=noisybwe lightning_datamodule.sensor=throat_microphone lightning_module=eben +callbacks=[bwe_checkpoint] ++"callbacks.checkpoint.monitor=validation/torchmetrics_stoi/synthetic" ++trainer.check_val_every_n_epoch=15 ++trainer.max_epochs=500 "trainer.devices=3\,"
```
