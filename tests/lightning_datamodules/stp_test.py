...
class TestSTPLightningDataModule:
    ...
    def test_tokenize_detokenize_is_bijection_from_dataloader(
        ...
        phonemes = stp_lightning_datamodule_instance.tokenizer.decode(
            token_ids=phonemes_ids
        )
        phonemes_ids_bis = stp_lightning_datamodule_instance.tokenizer(
            phonemes, add_special_tokens=True
        )
        phonemes_ids_bis = torch.Tensor(phonemes_ids_bis.input_ids)
        phonemes_bis = stp_lightning_datamodule_instance.tokenizer.decode(
            token_ids=phonemes_ids_bis
        )
