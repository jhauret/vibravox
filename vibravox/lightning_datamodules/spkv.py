...
class SPKVLightningDataModule(LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage == "test":
            ...
            pickle_file = f"configs/lightning_datamodule/spkv_pairs/{self.dataset_name.split("/")[1]}/{self.subset}/{self.pairs}.pkl"

            if not Path(__file__).parent.parent.parent /pickle_file.exists():
                raise ValueError(f"File {pickle_file} does not exist, please generate one for your dataset using scripts/gen_pairs_for_spkv.py")

            with open( Path(__file__).parent.parent.parent /pickle_file, 'rb') as file:
